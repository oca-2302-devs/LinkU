...
export function ProfileCards({ users }: { users: User[] }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* --- 詳細モーダル (selected がある時だけ表示) --- */}
      {selected && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          <div
            className="bg-white rounded-4xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()} // モーダル内クリックで閉じないように
          >
            <button
              className="absolute top-4 right-4 p-2 bg-white/80 backdrop-blur-md rounded-full text-gray-600 hover:text-black hover:bg-white transition-all z-20 shadow-lg border border-white/20"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {users.map((user) => (
          <div
            key={user.email} // uniqueなIDがあればそちらを推奨
